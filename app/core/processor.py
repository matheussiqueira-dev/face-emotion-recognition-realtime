...
class VideoProcessor:
    ...
    def __init__(self, config: AppConfig):
        ...
        self.analyzer = EmotionAnalyzer(enabled=config.enable_emotion_analysis)
    ...
    def run(self, callback: Optional[FrameCallback] = None):
        ...
        try:
            while not self._stop_requested.is_set():
                ...
                if not success:
                    if not source.isdigit():  # End of file/stream for non-camera source.
                        break
                    continue
                ...
                for track in tracks:
                    if (now - track.last_analyzed) >= self.config.emotion_interval:
                        ...
                        if face_img.size > 0:
                            ...
                            if emotion:
                                max_score = max(emotion.scores.values()) if emotion.scores else 0.0
                                if max_score >= self.config.min_emotion_score:
                ...
                if callback:
                    try:
                        ...
                        LOGGER.exception("Frame callback failed: %s", callback_error)
                ...
                # Frame pacing to cap CPU usage.
                target_frame_time = 1.0 / max(1, self.config.max_fps)
                elapsed = time.perf_counter() - frame_loop_start
